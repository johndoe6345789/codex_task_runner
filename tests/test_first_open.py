...
def test_first_open_found() -> None:
    with patch("codex_task_runner.pr.first_open.get_pr") as mock_get:
        mock_get.return_value = PullRequest(number=1, url="http://gh/1", title="T", author="a", state="open", mergeable="MERGEABLE", checks_state="SUCCESS")
...
def test_first_open_not_mergeable() -> None:
    with patch("codex_task_runner.pr.first_open.get_pr") as mock_get:
        mock_get.side_effect = [
            PullRequest(number=1, url="http://gh/1", title="T", author="a", state="open", mergeable="", checks_state=None),
            PullRequest(number=2, url="http://gh/2", title="T2", author="b", state="open", mergeable="MERGEABLE", checks_state="SUCCESS"),
        ]
        result = _first_open("owner/repo", [1, 2])
        assert result == 2

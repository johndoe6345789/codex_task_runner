...
from codex_task_runner.handlers import ping, poll, tasks, task, turns, create_pr, run, yolo, dedup_prs, discover, archive, patch, me, ui, prompt, serve
...
def _poll_codex_adapter(args, session):
    ...
    return str(cli_helpers.poll_codex_urls(env, getattr(args, "urls_file", "urls.txt"), getattr(args, "out", "run_poll.json")))


def _run_to_github_adapter(args, session):
    return cli_helpers.run_codex_to_github(getattr(args, "env", ".env"), getattr(args, "dry_run", False), getattr(args, "limit", 20))


# Wire new CLI commands to the adapters
_HANDLERS.update({
